...
from seeds_classification.data_utils import iterate_minibatches, data_augmentation
...
class prediction_net(object):
    ...
    def build_and_train(self, X_train, y_train, X_val=None, y_val=None,
        ...
        if save_model:
            ...
            with open(os.path.join(homedir, 'seeds_classification', 'training_info', filename + '.json'), 'w') as outfile:
                json.dump(train_info, outfile)
            np.savez(os.path.join(homedir, 'seeds_classification', 'training_weights', filename + '.npz'), *lasagne.layers.get_all_param_values(net['prob']))
